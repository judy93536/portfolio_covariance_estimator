...
#     # New sector-based selection method
#     getMostLiquidBySector = function(start_date, end_date, sectors = NULL, stocks_per_sector = 10) {
#       # Default to active sectors if none specified (using actual Sharadar sector names)
#       if (is.null(sectors)) {
#         sectors <- c("Technology", "Financial Services", "Healthcare", 
#                     "Consumer Cyclical", "Industrials", "Energy")
#       }
      
#       # Format sectors for SQL IN clause
#       sectors_sql <- paste0("'", sectors, "'", collapse = ", ")
...
#     # Method to explore available sectors
#     getAvailableSectors = function() {
#       self$connect()
#       query <- "
#         SELECT sector, COUNT(*) as count
#         FROM sharadar_src.tickers
#         WHERE isdelisted = 'False' AND sector IS NOT NULL
#         GROUP BY sector
#         ORDER BY count DESC"
...
#     # Method to get sector info for specific tickers
#     getSectorInfo = function(tickers) {
#       self$connect()
#       quoted <- paste0("'", tickers, "'", collapse = ", ")
#       query <- sprintf("
#         SELECT ticker, sector, industry, scalemarketcap
#         FROM sharadar_src.tickers
#         WHERE ticker IN (%s)", quoted)
